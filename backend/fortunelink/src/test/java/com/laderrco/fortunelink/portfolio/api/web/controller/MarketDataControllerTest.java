...
class MarketDataControllerTest {
  ...
  class Search {
    ...
    void returns200WithResults() throws Exception {
      when(marketDataService.searchSymbols("AAPL")).thenReturn(List.of(
          new SymbolSearchResult(new AssetSymbol("AAPL"), "Apple Inc.",
              "NASDAQ", Currency.of("USD"))));

      mockMvc.perform(get(BASE_URL + "/search").param("query", "AAPL"))
    ...
    void returns400WhenTooManySymbols() throws Exception {
      ...
      mockMvc.perform(post(BASE_URL + "/quotes/batch")
    ...
    void filtersOutInvalidSymbolsInBatch() throws Exception {
      ...
      when(marketDataService.getBatchQuotes(Set.of(msft)))
          .thenReturn(Map.of(msft, buildQuote("MSFT", "400.00")));
      ...
      mockMvc.perform(post(BASE_URL + "/quotes/batch")
          ...
          .andDo(print())
          .andExpect(status().isOk())
          .andExpect(jsonPath("$.MSFT").exists())
          .andExpect(jsonPath("$.length()").value(1)); 
  ...
  class GetQuote {
    ...
    void returns200WithQuote() throws Exception {
      ...
      when(marketDataService.getBatchQuotes(anySet()))
          .thenReturn(Map.of(symbol, buildQuote("AAPL", "185.50")));

      mockMvc.perform(get(BASE_URL + "/quotes/AAPL"))
          .andExpect(status().isOk())
    ...
    void returns400ForMalformedSymbol() throws Exception {
      mockMvc.perform(get(BASE_URL + "/quotes/bad symbol!"))
          .andExpect(status().isBadRequest());
    }
    ...
    void returns404WhenQuoteNotFound() throws Exception {
      ...
      when(marketDataService.getBatchQuotes(Set.of(assetSymbol)))
          .thenReturn(Map.of());
  ...
  class GetAssetInfo {
    ...
    void returns200WhenInfoExists() throws Exception {
      AssetSymbol symbol = new AssetSymbol("AAPL");
      MarketAssetInfo info = new MarketAssetInfo(symbol, "Apple Inc.", AssetType.STOCK, "NASDAQ", Currency.of("USD"),
          "Tech", "Desc");

      when(marketDataService.getAssetInfo(symbol)).thenReturn(Optional.of(info));

      mockMvc.perform(get(BASE_URL + "/info/AAPL"))
          .andExpect(status().isOk())
    ...
    void returns404WhenInfoMissing() throws Exception {
      ...
      mockMvc.perform(get(BASE_URL + "/info/UNKNOWN"))
          .andExpect(status().isNotFound())
  ...
  class ValidateSymbol {
    ...
    void returns200OnValidSymbol() throws Exception {
      ...
      mockMvc.perform(get(BASE_URL + "/validate/AAPL"))
          .andExpect(status().isOk())
          .andExpect(jsonPath("$.symbol").value("AAPL"));
    }
    ...
    void returns404OnUnknownSymbol() throws Exception {
      ...
      when(marketDataService.validateAndGet(symbol))
          .thenThrow(new UnknownSymbolException("Symbol not found"));

      mockMvc.perform(get(BASE_URL + "/validate/FAKE"))
          .andExpect(status().isNotFound())
    ...
    void returns400OnMalformedSymbol() throws Exception {
      ...
      mockMvc.perform(get(BASE_URL + "/validate/bad sym!"))
          .andExpect(status().isBadRequest())
  ...
  private MarketAssetQuote buildQuote(String symbol, String price) {
    ...
    return new MarketAssetQuote(
        new AssetSymbol(symbol), p, p, p, p, p,
        new PercentageChange(BigDecimal.ZERO), BigDecimal.ZERO,
        null, null, "FMP", Instant.now());
  }
}