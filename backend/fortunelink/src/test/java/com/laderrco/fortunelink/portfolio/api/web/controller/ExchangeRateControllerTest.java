...
class ExchangeRateControllerTest {
  ...
  class GetCurrentRate {
    ...
    void returns200WithRate() throws Exception {
      when(exchangeRateService.getRate(any(Currency.class), any(Currency.class)))
          .thenReturn(Optional.of(buildRate("USD", "CAD", "1.36")));
      ...
      mockMvc.perform(get(BASE_URL + "/current")
          ...
          .andExpect(jsonPath("$.to").value("CAD"))
          .andExpect(jsonPath("$.rate").value(1.36))
    ...
    void returns404OnExchangeRateUnavailableException() throws Exception {
      when(exchangeRateService.getRate(any(Currency.class), any(Currency.class)))
          .thenThrow(new ExchangeRateUnavailableException("USD", "AED", Instant.now()));
    ...
    void returns503WhenBocApiDown() throws Exception {
      when(exchangeRateService.getRate(any(Currency.class), any(Currency.class)))
          .thenThrow(new BocApiException("Connection refused"));
    ...
    void returns200ForCrossCurrencyPair() throws Exception {
      when(exchangeRateService.getRate(any(Currency.class), any(Currency.class)))
          .thenReturn(Optional.of(buildRate("EUR", "USD", "1.08")));
  ...
  class GetSupportedCurrencies {
    ...
    void returns200WithCurrencyList() throws Exception {
      mockMvc.perform(get(BASE_URL + "/supported"))
          .andExpect(status().isOk())
