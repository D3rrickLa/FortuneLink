...
class BocUrlBuilderTest {
  ...
  void shouldBuildBasicUrlWithObservations() {
    String result = builder.observations("FXUSDCAD")
        .build();
  ...
  void shouldBuildUrlWithMultipleSeries() {
    String result = builder.observations("FXUSDCAD", "FXEURCAD")
        .build();
  ...
  void shouldEncodeQueryParameters() {
    ...
    String result = new BocUrlBuilder(BASE_URL)
  ...
  void shouldHandleComplexFluentCall() {
    ...
    String result = builder.observations("FXUSDCAD", "FXGBPCAD")
        .format("csv")
        .startDate(date)
        .recent(1)
        .build();

    String expected = BASE_URL + "/observations/FXUSDCAD,FXGBPCAD/csv?start_date=2024-12-25&recent=1";
