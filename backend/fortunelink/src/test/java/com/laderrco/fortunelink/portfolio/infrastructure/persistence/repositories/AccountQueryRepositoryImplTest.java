...
class AccountQueryRepositoryImplTest {
  ...
  class InputValidation {
    ...
    void findQuantitiesShouldReturnEmptyMapWhenNull() {
      Map<AccountId, Map<AssetSymbol, Quantity>> result = repository.findQuantitiesForAccounts(null);
    ...
    void findQuantitiesShouldReturnEmptyMapWhenEmpty() {
      Map<AccountId, Map<AssetSymbol, Quantity>> result = repository.findQuantitiesForAccounts(List.of());
  ...
  class BasicQueries {
    ...
    void findByPortfolioIdShouldCallJpa() {
      ...
      when(jpaAccountRepository.findByPortfolioId(RAW_PORTFOLIO_ID, PAGEABLE)).thenReturn(expectedPage);
    ...
    void findByIdWithDetailsShouldMapResult() {
      ...
      when(jpaAccountRepository.findByIdWithOwnershipCheck(RAW_ACCOUNT_ID, RAW_PORTFOLIO_ID, RAW_USER_ID))
  ...
  class BatchGroupingQueries {
    ...
    void findQuantitiesShouldGroupRowsCorrectly() {
      ...
      AssetBalanceProjection row1 = new TestAssetBalanceProjection(RAW_ACCOUNT_ID, "BTC", new BigDecimal("1.5"));
      AssetBalanceProjection row2 = new TestAssetBalanceProjection(RAW_ACCOUNT_ID, "ETH", new BigDecimal("10.0"));

      when(jpaAccountRepository.findBalancesForAccounts(anyList()))
          .thenReturn(List.of(row1, row2));

      Map<AccountId, Map<AssetSymbol, Quantity>> result = repository.findQuantitiesForAccounts(List.of(ACCOUNT_ID));
    ...
    void findSymbolsForAccountsShouldGroupCorrectly() {
      ...
      Map<AccountId, Set<AssetSymbol>> result = repository.findSymbolsForAccounts(List.of(ACCOUNT_ID));
  ...
  private static class TestAssetBalanceProjection implements AssetBalanceProjection {
