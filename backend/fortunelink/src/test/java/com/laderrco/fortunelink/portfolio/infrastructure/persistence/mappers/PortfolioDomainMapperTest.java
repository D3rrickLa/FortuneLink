...
class PortfolioDomainMapperTest {
  ...
  class CollectionStrategy {
    ...
    void realizedGainsShouldUseAppendOnlyLogic() {
      ...
      RealizedGainJpaEntity existingGain = RealizedGainJpaEntity.create(
          existingGainId, existingAccount, "AAPL", BigDecimal.TEN, "USD",
      ...
      domainAccount.recordRealizedGain(
      ...
      AccountJpaEntity resultAccount = result.getAccounts().stream()
          .filter(a -> a.getId().equals(ACCOUNT_UUID))
          .findFirst()
          .orElseThrow();
  ...
  void positionsShouldBeFullyReplaced() {
    ...
    PortfolioJpaEntity existingPortfolio = createBasePortfolioEntity();
    AccountJpaEntity existingAccount = createBaseAccountEntity(existingPortfolio);
    PositionJpaEntity oldPos = PositionJpaEntity.create(
        UUID.randomUUID(), existingAccount, "MARKET", "AAPL", "STOCK",
        BigDecimal.TEN, BigDecimal.valueOf(100), "USD", Instant.now(), Instant.now());
    existingAccount.replacePositions(Set.of(oldPos));
    existingPortfolio.replaceAccounts(List.of(existingAccount));

    AcbPosition msftPos = new AcbPosition(
        new AssetSymbol("MSFT"), AssetType.STOCK, Currency.of("USD"),
        new Quantity(BigDecimal.ONE), new Money(BigDecimal.valueOf(200), Currency.of("USD")),
        Instant.now(), Instant.now());

    Account updatedAccount = Account.reconstitute(
        ACCOUNT_ID, "Trading", AccountType.NON_REGISTERED_INVESTMENT, Currency.of("USD"),
        PositionStrategy.FIFO, HealthStatus.HEALTHY, AccountLifecycleState.ACTIVE,
        null, Instant.now(), Instant.now(),
        new Money(BigDecimal.ZERO, Currency.of("USD")),
        Map.of(new AssetSymbol("MSFT"), msftPos),
        List.of());

    Portfolio updatedDomain = Portfolio.reconstitute(
        PORTFOLIO_ID, USER_ID, "Main Portfolio", "Desc",
        Map.of(updatedAccount.getAccountId(), updatedAccount),
        Currency.of("USD"), false, null, null, Instant.now(), Instant.now());

    PortfolioJpaEntity result = mapper.toEntity(updatedDomain, existingPortfolio);

    AccountJpaEntity resultAccount = result.getAccounts().stream()
        .filter(a -> a.getId().equals(RAW_ACCOUNT_ID))
        .findFirst()
        .orElseThrow();
  ...
  private PortfolioJpaEntity createBasePortfolioEntity() {
    return PortfolioJpaEntity.create(
        PORTFOLIO_UUID, USER_UUID, "Main Portfolio", "Desc", "USD", false, null, null, Instant.now(), Instant.now());
  }

  private AccountJpaEntity createBaseAccountEntity(PortfolioJpaEntity portfolio) {
    return AccountJpaEntity.create(
        ACCOUNT_UUID, portfolio, "Trading", "TFSA", "USD", "FIFO", "HEALTHY", "ACTIVE",
        BigDecimal.ZERO, "USD", null, Instant.now(), Instant.now());
  }

  private Portfolio createBaseDomainPortfolio() {
    Account account = Account.reconstitute(
        AccountId.fromString(ACCOUNT_UUID.toString()), "Trading", AccountType.TAXABLE_INVESTMENT, Currency.of("USD"),
        PositionStrategy.FIFO, HealthStatus.HEALTHY, AccountLifecycleState.ACTIVE, null,
        Instant.now(), Instant.now(), new Money(BigDecimal.ZERO, Currency.of("USD")), Map.of(), List.of());

    return Portfolio.reconstitute(
        PortfolioId.fromString(PORTFOLIO_UUID.toString()), UserId.fromString(USER_UUID.toString()),
        "Main Portfolio", "Desc", Map.of(account.getAccountId(), account), Currency.of("USD"),
