...
class RateLimitConfigTest {
  ...
  void shouldNotLoadConfigWhenDisabled() {
    contextRunner.withPropertyValues("fortunelink.rate-limit.enabled=false").run(context -> {
  ...
  void verifyGlobalBucketLimits() {
    contextRunner.withPropertyValues("fortunelink.rate-limit.global.requests-per-minute=10",
