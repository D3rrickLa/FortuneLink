...
import com.laderrco.fortunelink.portfolio_management.api.web.controllers.CacheAdminController;
...
class CacheAdminControllerTest {
    ...
    void clearCache_ShouldReturn404WhenMissing() throws Exception {
        ...
        mockMvc.perform(delete("/api/admin/cache/unknown"))
    ...
    void clearAllCaches_ShouldIterateAll() throws Exception {
        ...
        mockMvc.perform(delete("/api/admin/cache"))
                .andExpect(status().isOk())
    ...
    void clearCacheEntry_ShouldEvictKey() throws Exception {
        ...
        mockMvc.perform(delete("/api/admin/cache/prices/AAPL"))
                .andExpect(status().isOk())
    ...
    void evictPriceCache_ShouldUseEvictionService() throws Exception {
        mockMvc.perform(delete("/api/admin/cache/price/TSLA"))
                .andExpect(status().isOk())
    ...
    void warmCache_ShouldCallMarketDataService() throws Exception {
        mockMvc.perform(post("/api/admin/cache/warm"))
                .andExpect(status().isOk())
    ...
    void clearCacheEntry_ShouldReturn4xxWhenCacheNotFound() throws Exception {
        ...
        // 2. Act & Assert
        mockMvc.perform(delete("/api/admin/cache/{name}/{key}", invalidCache, "AAPL"))
