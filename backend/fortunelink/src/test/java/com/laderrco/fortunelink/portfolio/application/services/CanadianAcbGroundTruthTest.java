...
public class CanadianAcbGroundTruthTest {
	...
	void replayTransaction_sell_uses_cashDelta_for_cra_correct_proceeds() {
		Account account = new Account(AccountId.newId(), "test-account", AccountType.NON_REGISTERED_INVESTMENT, CAD,
		...
		Transaction buyTx = TransactionFixtures.buy()
				.accountId(account.getAccountId()).symbol(symbol.value()).quantity(100)
				.pricePerUnit(new Price(Money.of(100.00, "CAD"))).commission(9.99).build();
		...
		Transaction sellTx = TransactionFixtures.sell()
				.accountId(account.getAccountId()).symbol(symbol.value()).quantity(40)
				.pricePerUnit(new Price(Money.of(120.00, "CAD"))).commission(9.99).build();
	...
	class AcbPositionTests {
		...
		void buy_includes_commission_in_acb() {
			...
			assertThat(positionCraCorrect.totalCostBasis().amount())
					...
					.isEqualByComparingTo(
							positionBuggy.totalCostBasis().amount()
									.add(new BigDecimal("9.99")));
		}
		...
		void sell_proceeds_are_net_of_commission() {
			...
			assertThat(gainDifference)
					...
					.isEqualByComparingTo(new BigDecimal("9.99")
							.setScale(MONEY_PRECISION, ROUNDING_MODE)); // 4800.00-4790.01 = 9.99 applied to acb math
		}
		...
		void full_round_trip_two_buys_partial_sell() {
			...
			// BUY 1
			pos = ((ApplyResult<AcbPosition>) pos.buy(Quantity.of(100), Money.of(10009.99, "CAD"), T1))
					.newPosition();
			...
			// BUY 2
			pos = ((ApplyResult<AcbPosition>) pos.buy(Quantity.of(50), Money.of(5509.99, "CAD"), T2))
					.newPosition();
			...
			assertThat(sale.realizedGainLoss().amount().setScale(3, ROUNDING_MODE))
					...
					.isEqualByComparingTo(
							expectedGain.stripTrailingZeros().setScale(3, ROUNDING_MODE));
			...
			assertThat(remaining.totalCostBasis().amount().setScale(2, ROUNDING_MODE))
					...
					.isEqualByComparingTo(expectedRemainingAcb.stripTrailingZeros().setScale(2,
							ROUNDING_MODE));
		}
	}
	...
	class FifoPositionTests {
		...
		void fifo_partial_lot_no_rounding_leak() {
			...
			pos = ((ApplyResult<FifoPosition>) pos.buy(Quantity.of(33), lotCost, T1))
					.newPosition();

			Money netProceeds = Money.of(400.01, "CAD"); // 10 × $41.00 - 9.99
			ApplyResult.Sale<FifoPosition> sale = (ApplyResult.Sale<FifoPosition>) pos.sell(Quantity.of(10),
					netProceeds, T2);
	...
	class ReplayContractTests {
		...
		void cashDelta_is_correct_for_cra_sell() {
			...
			ApplyResult.Sale<AcbPosition> withCashDelta = (ApplyResult.Sale<AcbPosition>) pos
					.sell(Quantity.of(40), cashDeltaNet, T2);

			// Simulate replayFullTransaction (current code — grossValue)
			Money grossValue = Money.of(4800.00, "CAD"); // 40 × 120, ignores commission
			ApplyResult.Sale<AcbPosition> withGrossValue = (ApplyResult.Sale<AcbPosition>) pos
					.sell(Quantity.of(40), grossValue, T2);
