...
class TransactionServiceTest {
  ...
  class RecordPurchaseTests {
    ...
    void recordPurchase_Success_NoCurrencyConversionNeeded() {
      var command = new RecordPurchaseCommand(portfolioId, userId, accountId, "AAPL",
          Quantity.of(10.0), Price.of(new BigDecimal("150.0"), USD), List.of(), Instant.now(),
          "Buy Apple");
      ...
      when(marketDataService.getAssetInfo(any())).thenReturn(Optional.of(
          new MarketAssetInfo(new AssetSymbol("AAPL"), "Apple", AssetType.STOCK, "NASDAQ", USD,
              "Tech", "some description here")));
    ...
    void recordPurchase_Failure_WhenSymbolIsUnknown() {
      var command = new RecordPurchaseCommand(portfolioId, userId, accountId, "UNKNOWN",
          Quantity.of(1), new Price(new Money(BigDecimal.ONE, USD)), null, Instant.now(), "notes");
      ...
      assertThatThrownBy(() -> transactionService.recordPurchase(command)).isInstanceOf(
  ...
  class RecordSaleTests {
    ...
    void recordSale_Failure_WhenNoPositionExists() {
      var command = new RecordSaleCommand(portfolioId, userId, accountId, "TSLA", Quantity.of(5.0),
          new Price(new Money(BigDecimal.TEN, USD)), List.of(), Instant.now(), "notes");
      ...
      assertThatThrownBy(() -> transactionService.recordSale(command)).isInstanceOf(
    ...
    void recordSale_Success_WhenCurrencyDiffers() {
      var command = new RecordSaleCommand(portfolioId, userId, accountId, "TSLA", Quantity.of(1.0),
      ...
      when(exchangeRateService.convertToPrice(any(), eq(USD))).thenReturn(
          Price.of(new BigDecimal("75.0"), USD));
  ...
  class ExcludeTransactionTests {
    ...
    void excludeTransaction_Failure_WhenTransactionIsAlreadyExcluded() {
      ...
      assertThatThrownBy(() -> transactionService.excludeTransaction(command)).isInstanceOf(
          InvalidTransactionException.class).hasMessageContaining("already excluded");
    }
  }
  ...
  class CashAndDividendTests {
    ...
    void recordDeposit_Success_CallsRecordingService() {
      ...
      when(portfolioRepository.findByIdAndUserId(portfolioId, userId)).thenReturn(
          Optional.of(portfolio));
      when(portfolio.getAccount(accountId)).thenReturn(account);
      when(transactionRecordingService.recordDeposit(any(), any(), any(), any())).thenReturn(
          transaction);
  ...
  class DividendReinvestmentTests {
    ...
    void recordDividendReinvestment_Success_WithNestedExecution() {
      ...
      var command = new RecordDividendReinvestmentCommand(portfolioId, userId, accountId, "AAPL",
          execution, Instant.now(), "DRIP");
  ...
  class RestoreTransactionTests {
    ...
    void restoreTransaction_Failure_WhenTransactionIsAlreadyActive() {
      ...
      assertThatThrownBy(() -> transactionService.restoreTransaction(command)).isInstanceOf(
          InvalidTransactionException.class).hasMessageContaining("not excluded");
    }
  }
  ...
  class OtherTransactionTypesTests {
    ...
    void recordWithdrawal_Success_CallsRecordingService() {
      var command = new RecordWithdrawalCommand(portfolioId, userId, accountId, Money.of(10, "USD"),
          Instant.now(), null);
      Portfolio portfolio = mock(Portfolio.class);
      Account account = mock(Account.class);
      Transaction transaction = mock(Transaction.class);
      ...
      verify(transactionRecordingService).recordWithdrawal(eq(account), eq(Money.of(10, "USD")),
          any(), any());
    }
    ...
    void recordFee_Success_CallsRecordingService() {
      ...
      when(portfolioRepository.findByIdAndUserId(portfolioId, userId)).thenReturn(
          Optional.of(portfolio));
      when(portfolio.getAccount(accountId)).thenReturn(account);
      when(transactionRecordingService.recordFee(any(), any(), any(), any())).thenReturn(
          transaction);

      transactionService.recordFee(command);

      verify(transactionRecordingService).recordFee(eq(account), eq(new Money(BigDecimal.ONE, USD)),
          any(), any());
    }
  }
  ...
  class ValidationTests {
    ...
    void validate_Failure_WhenValidatorReturnsErrors() {
      var command = new RecordDepositCommand(portfolioId, userId, accountId, Money.of(10, "USD"),
          null, Instant.now(), null);
      when(validator.validate(command)).thenReturn(
          ValidationResult.failure(List.of("Amount must be positive")));

      assertThatThrownBy(() -> transactionService.recordDeposit(command)).isInstanceOf(
          InvalidTransactionException.class).hasMessageContaining("Invalid recordDeposit command");
    }
  }
  ...
  class PortfolioContextTests {
    ...
    void getPortfolioContext_Failure_WhenRepositoryReturnsEmpty() {
      ...
      assertThatThrownBy(() -> transactionService.recordFee(command)).isInstanceOf(
