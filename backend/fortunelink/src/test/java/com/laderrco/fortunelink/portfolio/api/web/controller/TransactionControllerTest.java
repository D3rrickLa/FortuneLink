 ...
 *
 * <p>
 * TransactionController has 14 write endpoints, 2 read endpoints, and 2
 * exclusion
 * endpoints. This suite covers:
 * - All write endpoints: happy path 201 + key validation failure
 * - Both read endpoints: 200, 404, pagination
 * - Both exclusion endpoints: 200, 409, 404
 * - Idempotency-Key header handling (present and absent)
 */
...
class TransactionControllerTest {
  ...
  class RecordBuy {
    ...
    void returns201OnSuccess() throws Exception {
      ...
      mockMvc.perform(post(BASE_URL + "/buy")
          .header("Idempotency-Key", IDEMPOTENCY_KEY)
          .contentType(MediaType.APPLICATION_JSON)
          .content(validBuyRequest()))
    ...
    void returns422ForInsufficientFunds() throws Exception {
      when(transactionService.recordPurchase(any()))
          .thenThrow(new InsufficientFundsException("Insufficient cash for buy"));
  ...
  class RecordSell {
    ...
    void returns201OnSuccess() throws Exception {
      ...
      mockMvc.perform(post(BASE_URL + "/sell")
          .contentType(MediaType.APPLICATION_JSON)
          .content(validSellRequest()))
          .andExpect(status().isCreated());
    }
    ...
    void returns422ForInsufficientQuantity() throws Exception {
      when(transactionService.recordSale(any()))
          .thenThrow(new InsufficientQuantityException("Cannot sell 100, only holding 10"));
  ...
  class RecordReturnOfCapital {
    ...
    void returns201OnSuccess() throws Exception {
      when(transactionService.recordReturnOfCapital(any()))
          .thenReturn(buildTxView(TransactionType.RETURN_OF_CAPITAL));

      mockMvc.perform(post(BASE_URL + "/return-of-capital")
          .contentType(MediaType.APPLICATION_JSON)
          .content("""
              {
                  "assetSymbol": "VTI",
  ...
  class RecordDeposit {
    ...
    void returns201OnSuccess() throws Exception {
      when(transactionService.recordDeposit(any())).thenReturn(buildTxView(TransactionType.DEPOSIT));

      mockMvc.perform(post(BASE_URL + "/deposit")
          .contentType(MediaType.APPLICATION_JSON)
          .content(validCashRequest("1000.00")))
          .andExpect(status().isCreated());
    }
  ...
  class RecordWithdrawal {
    ...
    void returns201OnSuccess() throws Exception {
      when(transactionService.recordWithdrawal(any())).thenReturn(buildTxView(TransactionType.WITHDRAWAL));

      mockMvc.perform(post(BASE_URL + "/withdrawal")
          .contentType(MediaType.APPLICATION_JSON)
          .content(validCashRequest("500.00")))
          .andExpect(status().isCreated());
    }
    ...
    void returns422ForInsufficientFunds() throws Exception {
      when(transactionService.recordWithdrawal(any()))
          .thenThrow(new InsufficientFundsException("Insufficient cash"));

      mockMvc.perform(post(BASE_URL + "/withdrawal")
          .contentType(MediaType.APPLICATION_JSON)
          .content(validCashRequest("999999.00")))
          .andExpect(status().isUnprocessableContent());
    }
  }
  ...
  class RecordFee {
    ...
    void returns201OnSuccess() throws Exception {
      ...
      mockMvc.perform(post(BASE_URL + "/fee")
          .contentType(MediaType.APPLICATION_JSON)
          .content("""
              {
                  "amount": 10.00,
  ...
  class RecordInterest {
    ...
    void returns201ForCashInterest() throws Exception {
      when(transactionService.recordInterest(any())).thenReturn(buildTxView(TransactionType.INTEREST));

      mockMvc.perform(post(BASE_URL + "/interest")
          .contentType(MediaType.APPLICATION_JSON)
          .content("""
  ...
  class RecordDividend {
    ...
    void returns201OnSuccess() throws Exception {
      when(transactionService.recordDividend(any())).thenReturn(buildTxView(TransactionType.DIVIDEND));

      mockMvc.perform(post(BASE_URL + "/dividend")
          .contentType(MediaType.APPLICATION_JSON)
          .content("""
              {
                  "assetSymbol": "AAPL",
  ...
  class RecordDrip {
    ...
    void returns201OnSuccess() throws Exception {
      when(transactionService.recordDividendReinvestment(any()))
          .thenReturn(buildTxView(TransactionType.DIVIDEND_REINVEST));

      mockMvc.perform(post(BASE_URL + "/drip")
          .contentType(MediaType.APPLICATION_JSON)
          .content("""
              {
                  "assetSymbol": "AAPL",
  ...
  class Transfers {
    ...
    void returns201OnTransferIn() throws Exception {
      when(transactionService.recordTransferIn(any()))
          .thenReturn(buildTxView(TransactionType.TRANSFER_IN));

      mockMvc.perform(post(BASE_URL + "/transfer-in")
          .contentType(MediaType.APPLICATION_JSON)
          .content("""
    ...
    void returns201OnTransferOut() throws Exception {
      when(transactionService.recordTransferOut(any()))
          .thenReturn(buildTxView(TransactionType.TRANSFER_OUT));

      mockMvc.perform(post(BASE_URL + "/transfer-out")
          .contentType(MediaType.APPLICATION_JSON)
          .content("""
  ...
  class GetTransactionHistory {
    ...
    void returns200WithHistory() throws Exception {
      ...
      when(transactionQueryService.getTransactionHistory(any(GetTransactionHistoryQuery.class)))
          .thenReturn(page);
  ...
  class GetTransaction {
    ...
    void returns200OnSuccess() throws Exception {
      when(transactionQueryService.getTransaction(any(GetTransactionByIdQuery.class)))
          .thenReturn(buildTxView(TransactionType.BUY));

      mockMvc.perform(get(BASE_URL + "/" + TX_ID))
          .andExpect(status().isOk());
    }
    ...
    void returns404WhenNotFound() throws Exception {
      when(transactionQueryService.getTransaction(any(GetTransactionByIdQuery.class)))
          .thenThrow(new TransactionNotFoundException(TransactionId.fromString(TX_ID)));
  ...
  class ExcludeTransaction {
    ...
    void returns200OnSuccess() throws Exception {
      when(transactionService.excludeTransaction(any()))
          .thenReturn(buildTxView(TransactionType.BUY));

      mockMvc.perform(patch(BASE_URL + "/" + TX_ID + "/exclude")
          .header("Idempotency-Key", IDEMPOTENCY_KEY)
          .contentType(MediaType.APPLICATION_JSON)
          .content("{\"reason\": \"Duplicate import\"}"))
          .andExpect(status().isOk());
    }
  ...
  class RestoreTransaction {
    ...
    void returns200OnSuccess() throws Exception {
      when(transactionService.restoreTransaction(any()))
          .thenReturn(buildTxView(TransactionType.BUY));

      mockMvc.perform(patch(BASE_URL + "/" + TX_ID + "/restore")
          .header("Idempotency-Key", IDEMPOTENCY_KEY))
          .andExpect(status().isOk());
    }
  ...
  private TransactionView buildTxView(TransactionType type) {
    ...
    return new TransactionView(
        TransactionId.newId(),
        type,
        type.affectsHoldings() ? "AAPL" : null,
        type.affectsHoldings() ? new Quantity(BigDecimal.TEN) : null,
        type.affectsHoldings() ? Price.of(BigDecimal.valueOf(150), cad) : null,
        List.of(),
        Money.zero(cad),
        Map.of(),
        Instant.now(),
  ...
  /**
   * Valid BUY body — includes accountId as it is @NotNull on
