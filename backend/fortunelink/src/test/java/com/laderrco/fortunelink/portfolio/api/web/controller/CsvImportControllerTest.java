...
class CsvImportControllerTest {
  ...
  class ImportCsv {
    ...
    void returns200OnSuccessfulImport() throws Exception {
      when(csvImportService.importTransactions(any(), any(), any(), any()))
          .thenReturn(CsvImportResult.success(5));
    ...
    void returns422OnValidationErrors() throws Exception {
      List<CsvRowError> errors = List.of(
          new CsvRowError(2, "Invalid date format"),
          new CsvRowError(4, "Symbol required for BUY"));
      when(csvImportService.importTransactions(any(), any(), any(), any()))
          .thenReturn(CsvImportResult.failure(errors));
    ...
    void returns400ForEmptyFile() throws Exception {
      ...
      mockMvc.perform(multipart(BASE_URL).file(emptyFile))
    ...
    void returns413ForOversizedFile() throws Exception {
      ...
      mockMvc.perform(multipart(BASE_URL).file(bigFile))
    ...
    void accepts200ForOctetStream() throws Exception {
      when(csvImportService.importTransactions(any(), any(), any(), any()))
          .thenReturn(CsvImportResult.success(3));

      MockMultipartFile file = new MockMultipartFile(
          "file", "data.csv", "application/octet-stream",
    ...
    void accepts200ForTextPlain() throws Exception {
      when(csvImportService.importTransactions(any(), any(), any(), any()))
          .thenReturn(CsvImportResult.success(2));
    ...
    void returns500WhenCommitFails() throws Exception {
      when(csvImportService.importTransactions(any(), any(), any(), any()))
          .thenThrow(new CsvImportCommitException("Row 3 failed on commit: insufficient cash", null));
    ...
    void accepts200ForNullContentType() throws Exception {
      when(csvImportService.importTransactions(any(), any(), any(), any()))
          .thenReturn(CsvImportResult.success(1));

      MockMultipartFile file = new MockMultipartFile(
          "file", "data.csv", (String) null,
          validCsvContent().getBytes());
  ...
  private MockMultipartFile validCsvFile() {
    return new MockMultipartFile(
        "file", "import.csv", "text/csv", validCsvContent().getBytes());
  }
