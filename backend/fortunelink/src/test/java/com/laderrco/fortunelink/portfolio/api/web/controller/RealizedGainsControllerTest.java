...
class RealizedGainsControllerTest {
  ...
  private static final String BASE_URL =
      "/api/v1/portfolios/" + PORTFOLIO_ID + "/accounts/" + ACCOUNT_ID + "/realized-gains";
  ...
  private RealizedGainsSummaryView buildSummaryView(Integer taxYear) {
    ...
    return new RealizedGainsSummaryView(List.of(), zero, zero, zero, cad, taxYear, 0L, 0);
  }
  ...
  class GetRealizedGains {
    ...
    void returns200WithAllTimeGains() throws Exception {
      when(realizedGainsQueryService.getRealizedGains(any(GetRealizedGainsQuery.class))).thenReturn(
          buildSummaryView(null));

      mockMvc.perform(get(BASE_URL)).andExpect(status().isOk())
          .andExpect(jsonPath("$.items").isArray()).andExpect(jsonPath("$.currency").value("CAD"))
    ...
    void returns200WithYearFilter() throws Exception {
      when(realizedGainsQueryService.getRealizedGains(any(GetRealizedGainsQuery.class))).thenReturn(
          buildSummaryView(2024));

      mockMvc.perform(get(BASE_URL).param("taxYear", "2024")).andExpect(status().isOk())
    ...
    void returns200WithSymbolFilter() throws Exception {
      ...
      ArgumentCaptor<GetRealizedGainsQuery> captor = ArgumentCaptor.forClass(
          GetRealizedGainsQuery.class);
    ...
    void returns200WithCombinedFilters() throws Exception {
      ...
      mockMvc.perform(get(BASE_URL).param("taxYear", "2024").param("symbol", "AAPL"))
    ...
    void returns400ForTaxYearTooEarly() throws Exception {
      mockMvc.perform(get(BASE_URL).param("taxYear", "1999")).andExpect(status().isBadRequest());
    ...
    void returns400ForLowercaseSymbol() throws Exception {
      ...
      mockMvc.perform(get(BASE_URL).param("symbol", "aapl")).andExpect(status().isBadRequest());
    ...
    void returns404WhenNotFound() throws Exception {
      when(realizedGainsQueryService.getRealizedGains(any(GetRealizedGainsQuery.class))).thenThrow(
    ...
    void getRealizedGainsWithFutureTaxYearShouldReturnBadRequest() throws Exception {
      ...
      mockMvc.perform(get(BASE_URL).param("taxYear", String.valueOf(futureYear)).with(user("test")))
          .andExpect(status().isBadRequest());
    ...
    void getRealizedGains_WithBlankSymbol_ShouldPassNullAssetSymbol() throws Exception {
      when(realizedGainsQueryService.getRealizedGains(any())).thenReturn(buildSummaryView(2025));

      mockMvc.perform(get(BASE_URL).with(user("test"))).andExpect(status().isOk());
