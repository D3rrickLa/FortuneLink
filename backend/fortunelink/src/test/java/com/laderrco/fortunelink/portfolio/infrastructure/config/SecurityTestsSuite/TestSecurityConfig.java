...
public class TestSecurityConfig {
  ...
  /**
   * Provides a mock JwtDecoder so the context loads without hitting Supabase. This bean is ONLY
   * registered when this config is imported.
   */
  @Bean
  @Primary
  public JwtDecoder jwtDecoder() {
    return token -> {
      ...
      return Jwt.withTokenValue(token).header("alg", "RS256").subject(UUID.randomUUID().toString())
          .claim("email", "test@fortunelink.com").issuedAt(Instant.now())
