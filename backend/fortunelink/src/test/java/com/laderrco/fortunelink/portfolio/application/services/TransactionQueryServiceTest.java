...
public class TransactionQueryServiceTest {
  ...
  class GetTransactionTests {
    ...
    void getTransactionThrowsNotFound() {
      ...
      assertThatThrownBy(() -> transactionQueryService.getTransaction(query))
    ...
    void getTransactionReturnsView() {
      ...
      when(transactionRepository.findByIdAndPortfolioIdAndUserIdAndAccountId(
          TX_ID, PORTFOLIO_ID, USER_ID, ACCOUNT_ID)).thenReturn(Optional.of(mockTx));
  ...
  class GetTransactionHistoryTests {
    ...
    void getTransactionHistoryUsesDateRange() {
      ...
      when(transactionQueryRepository.findByAccountIdAndDateRange(eq(ACCOUNT_ID), eq(START), eq(END), any()))
          .thenReturn(mockPage);

      transactionQueryService.getTransactionHistory(query);

      verify(transactionQueryRepository).findByAccountIdAndDateRange(eq(ACCOUNT_ID), eq(START), eq(END), any());
    }
    ...
    void getTransactionHistoryUsesSymbol() {
      ...
      when(transactionQueryRepository.findByAccountIdAndSymbol(eq(ACCOUNT_ID), eq(SYMBOL), any()))
          .thenReturn(mockPage);

      transactionQueryService.getTransactionHistory(query);

      verify(transactionQueryRepository).findByAccountIdAndSymbol(eq(ACCOUNT_ID), eq(SYMBOL), any());
    }
    ...
    class GetTransactionHistoryPartialFilterTests {
      ...
      void getTransactionHistoryDefaultsToBasicWhenOnlyEndDateProvided() {
        ...
        when(transactionQueryRepository.findByAccountId(eq(ACCOUNT_ID), any()))
            .thenReturn(mockPage);
  ...
  class InternalCalculationTests {
    ...
    void getTransactionsForCalculationValidatesOwnership() {
      ...
      // portfolio)
      Mockito.doThrow(new SecurityException("Unauthorized"))
          .when(portfolioLoader).validatePortfolioAndAccountOwnership(PORTFOLIO_ID, USER_ID, ACCOUNT_ID);
    ...
    void getTransactionsForCalculationGuardChecks() {
      ...
      assertThatThrownBy(() -> transactionQueryService.getTransactionsForCalculation(nullQuery))
  ...
  private GetTransactionHistoryQuery createHistoryQuery(Instant start, Instant end, AssetSymbol symbol) {
