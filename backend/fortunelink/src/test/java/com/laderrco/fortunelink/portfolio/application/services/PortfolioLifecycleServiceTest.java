...
class PortfolioLifecycleServiceTest {
  ...
  class CreatePortfolioTests {
    ...
    void setup() {
      ...
      lenient().when(validator.validate((CreatePortfolioCommand) any()))
    ...
    void createPortfolio_Failure_ValidationFails() {
      ...
      when(validator.validate(command)).thenReturn(
          ValidationResult.failure(List.of("Portfolio name is required")));

      assertThatThrownBy(() -> service.createPortfolio(command)).isInstanceOf(
          InvalidCommandException.class).hasMessageContaining("Invalid createPortfolio command");
    }
    ...
    void createPortfolio_Failure_InvalidCommand() {
      ...
      when(validator.validate(command)).thenReturn(
          ValidationResult.failure(Collections.singletonList("Name required")));
  ...
  class UpdatePortfolioTests {
    ...
    void setup() {
      // Default behavior for validator to avoid stubbing in every success test
      lenient().when(validator.validate((UpdatePortfolioCommand) any()))
          .thenReturn(ValidationResult.success());
    }
    ...
    void updatePortfolio_Success_ValidIdAndCommand() {
      ...
      when(validator.validate(any(UpdatePortfolioCommand.class))).thenReturn(
  ...
  class DeletePortfolioTests {
    ...
    void setup() {
      // Default behavior for validator to avoid stubbing in every success test
      lenient().when(validator.validate((DeletePortfolioCommand) any()))
          .thenReturn(ValidationResult.success());
    }
    ...
    void deletePortfolio_Failure_WhenAlreadyDeleted() {
      ...
      assertThatThrownBy(() -> service.deletePortfolio(command)).isInstanceOf(
          PortfolioDeletionException.class).hasMessageContaining("Portfolio already deleted");
    }
    ...
    void deletePortfolio_Failure_WhenPortfolioIsNotEmpty() {
      ...
      doThrow(new PortfolioDeletionException("Cannot delete non-empty portfolio")).when(portfolio)
          .markAsDeleted(userId);
    ...
    void deletePortfolio_Failure_WhenIllegalStateOccurs() {
      ...
      doThrow(new IllegalStateException("Unknown state conflict")).when(portfolio)
          .markAsDeleted(userId);
  ...
  class AccountManagementTests {
    ...
    void createAccount_Success_ValidCommand() {
      lenient().when(validator.validate((CreateAccountCommand) any()))
          .thenReturn(ValidationResult.success());
    ...
    void updateAccount_Success_Updates() {
      lenient().when(validator.validate((UpdateAccountCommand) any()))
          .thenReturn(ValidationResult.success());
    ...
    void deleteAccount_Success_ValidIds() {
      lenient().when(validator.validate((DeleteAccountCommand) any()))
          .thenReturn(ValidationResult.success());
      var accountId = AccountId.newId();
