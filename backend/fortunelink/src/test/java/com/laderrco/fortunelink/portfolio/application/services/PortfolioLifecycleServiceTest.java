...
class PortfolioLifecycleServiceTest {
	...
	void setUp() {
		...
		lenient().when(validator.validate(any(CreateAccountCommand.class))).thenReturn(ValidationResult.success());
		lenient().when(validator.validate(any(UpdateAccountCommand.class))).thenReturn(ValidationResult.success());
		lenient().when(validator.validate(any(ReopenAccountCommand.class))).thenReturn(ValidationResult.success());
		lenient().when(validator.validate(any(DeleteAccountCommand.class))).thenReturn(ValidationResult.success());
	}
	...
	class CreatePortfolioTests {
		...
		void createsDefaultAccount() {
			...
			CreatePortfolioCommand command = new CreatePortfolioCommand(USER_ID, "Name", "Desc", USD, true,
					AccountType.MARGIN, PositionStrategy.ACB);
		...
		void createsDefaultAccountThrowsEexceptionFromCreatePortfolioCOmmand() {
			assertThatThrownBy(() -> new CreatePortfolioCommand(USER_ID, "Name", "Desc", USD, true,
					null, PositionStrategy.ACB))
		...
		void mapsDataIntegrityException() {
			when(portfolioRepository.save(any())).thenThrow(new DataIntegrityViolationException("Unique constraint"));

			CreatePortfolioCommand command = new CreatePortfolioCommand(USER_ID, "Name", "Desc", USD, false, null,
		...
		void createsDefaultAccountThrowExceptionWithValidation() {
			when(validator.validate(any(CreatePortfolioCommand.class)))
					.thenReturn(ValidationResult.failure("Everything is null"));
			CreatePortfolioCommand command = new CreatePortfolioCommand(USER_ID, "Name", "Desc", USD, true,
					AccountType.MARGIN, PositionStrategy.ACB);
			assertThatThrownBy(() -> service.createPortfolio(command))
	...
	class UpdatePortfolioTests {
		...
		void skipsFeesWhenNoAccounts() {
			...
			service.updatePortfolio(new UpdatePortfolioCommand(PORTFOLIO_ID, USER_ID, "Name", "Desc", USD));

			// Verify fee repository is never touched
			verify(transactionRepository, never()).sumBuyFeesByAccountAndSymbol(any());
			verify(portfolioViewMapper).toPortfolioView(eq(mockPortfolio), anyList(), any(), anyBoolean());
		}
		...
		void verifiesTransactionBlockLogic() {
			...
			service.updatePortfolio(new UpdatePortfolioCommand(PORTFOLIO_ID, USER_ID, "New Name", "New Desc", USD));
		...
		void throwsExceptionOnNullTransactionResult() {
			...
			UpdatePortfolioCommand command = new UpdatePortfolioCommand(PORTFOLIO_ID, USER_ID, "New Name", "DESC", USD);
		...
		void handlesMarketDataFailureGracefully() {
			...
			when(transactionTemplate.execute(any())).thenReturn(new UpdatePortfolioResult(mockPortfolio, List.of(accId)));

			// Mock market data failure
			when(marketDataService.getBatchQuotes(anySet())).thenThrow(new RuntimeException("External Service Down"));

			UpdatePortfolioCommand command = new UpdatePortfolioCommand(PORTFOLIO_ID, USER_ID, "Update", "DESC2", USD);
		...
		void propagatesStaleFlagFromAccounts() {
			...
			when(mockPortfolio.getDisplayCurrency()).thenReturn(USD);
			when(transactionTemplate.execute(any())).thenReturn(new UpdatePortfolioResult(mockPortfolio, List.of(accountId)));

			when(marketDataService.getBatchQuotes(any())).thenReturn(Map.of());
			when(portfolioValuationService.calculateTotalValue(any(), any(), any()))
					.thenReturn(Money.zero("USD"));

			service.updatePortfolio(new UpdatePortfolioCommand(PORTFOLIO_ID, USER_ID, "Update", "DESC2", USD));
		...
		void continuesOnFeeFetchError() {
			...
			when(transactionTemplate.execute(any())).thenReturn(new UpdatePortfolioResult(mockPortfolio, List.of(accId)));
			when(transactionRepository.sumBuyFeesByAccountAndSymbol(any())).thenThrow(new RuntimeException("DB Error"));
	...
	class DeletePortfolioTests {
		...
		void recursiveDeleteFailsWithNonEmptyAccounts() {
			...
			DeletePortfolioCommand command = new DeletePortfolioCommand(PORTFOLIO_ID, USER_ID, true, true, true);
		...
		void throwsWhenNotFound() {
			...
			DeletePortfolioCommand cmd = new DeletePortfolioCommand(PORTFOLIO_ID, USER_ID, true, true, false);
		...
		void mapsNotEmptyException() {
			...
			doThrow(new PortfolioNotEmptyException("Close accounts first")).when(portfolio).markAsDeleted(USER_ID);

			DeletePortfolioCommand cmd = new DeletePortfolioCommand(PORTFOLIO_ID, USER_ID, true, true, false);
			assertThatThrownBy(() -> service.deletePortfolio(cmd))
					.isInstanceOf(PortfolioDeletionException.class)
					.hasMessageContaining("close accounts first");
		}
		...
		void mapsAlreadyDeletedException() {
			...
			doThrow(new PortfolioAlreadyDeletedException("Already gone"))
					.when(portfolio).markAsDeleted(USER_ID);

			DeletePortfolioCommand cmd = new DeletePortfolioCommand(PORTFOLIO_ID, USER_ID, true, true, false);

			assertThatThrownBy(() -> service.deletePortfolio(cmd))
					.isInstanceOf(PortfolioDeletionException.class)
					.hasMessage("Already gone");
		}
		...
		void mapsIllegalStateException() {
			...
			doThrow(new IllegalStateException("Inconsistent state"))
					.when(portfolio).markAsDeleted(USER_ID);
	...
	class AccountOperationTests {
		...
		void createAccountSuccess() {
			...
			service.createAccount(
					new CreateAccountCommand(PORTFOLIO_ID, USER_ID, "Taxable", AccountType.CHEQUING, PositionStrategy.ACB, USD));
		...
		void reopenAccountErrorMapping() {
			...
			ReopenAccountCommand command = new ReopenAccountCommand(AccountId.newId(), PORTFOLIO_ID, USER_ID);

			assertThatThrownBy(() -> service.reopenAccount(command))
					.isInstanceOf(AccountCannotBeReopenedException.class);
		}
		...
		void deleteAccountThrowsException() {
			...
			assertThatThrownBy(() -> service.deleteAccount(command))
					.isInstanceOf(AccountCannotBeClosedException.class);
		}
	}
}