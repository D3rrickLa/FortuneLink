...
class TransactionDomainMapperTest {
  ...
  class ExclusionMutations {
    ...
    void applyExclusionStateShouldUpdateManagedEntity() {
      ...
      Transaction domain = Transaction.builder()
          ...
          .cashDelta(Money.of(-1000, Currency.CAD))
          .notes("NOTES")
          .execution(new TradeExecution(new AssetSymbol("AAPL"), Quantity.of(10), Price.of("100", Currency.CAD)))
          .transactionId(TransactionId.fromString(TX_UUID.toString()))
          .metadata(new TransactionMetadata(AssetType.STOCK, "MANUAL",
              new TransactionMetadata.ExclusionRecord(Instant.now(), UserId.fromString(USER_UUID.toString()),
                  "Reversed"),
              null))
          .build();
  ...
  private TransactionJpaEntity createBaseJpaEntity() {
    return TransactionJpaEntity.create(
        TX_UUID, PORTFOLIO_UUID, ACCOUNT_UUID, "BUY", null, null, null, null, "STOCK",
        null, null, BigDecimal.ZERO, "USD", "MANUAL", false, null, null, null, null,
        "Notes", Instant.now(), null, IDEM_KEY);
  }

  private Transaction createDomainBuyTransaction() {
    TradeExecution exec = new TradeExecution(
        new AssetSymbol("MSFT"), new Quantity(BigDecimal.ONE),
