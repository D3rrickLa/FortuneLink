...
(
    ...
    total_liabilities_currency VARCHAR(3)      NOT NULL,
...
CREATE INDEX idx_snapshots_user_date
    ON valuation_snapshots (user_id, snapshot_date DESC);
...
CREATE UNIQUE INDEX idx_snapshots_user_day
    ON valuation_snapshots (user_id, CAST(snapshot_date AT TIME ZONE 'UTC' AS DATE));

COMMENT
ON TABLE valuation_snapshots IS
