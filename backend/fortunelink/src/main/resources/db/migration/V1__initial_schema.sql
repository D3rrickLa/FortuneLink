...
COMMENT
ON COLUMN accounts.position_strategy IS
    'ACB is the only supported strategy at MVP. FIFO stub exists for future use.';
 
COMMENT
ON COLUMN accounts.health_status IS
...
COMMENT
ON COLUMN positions.identifier_type IS
...
COMMENT
ON COLUMN transactions.portfolio_id IS
...
COMMENT
ON COLUMN transactions.additional_data IS
...
(
    ...
    CONSTRAINT chk_conversion_completeness CHECK (
        (account_amount IS NULL AND account_amount_currency IS NULL
            AND exchange_rate IS NULL AND rate_from_currency IS NULL AND
         rate_to_currency IS NULL) OR
        (account_amount IS NOT NULL AND account_amount_currency IS NOT NULL
            AND exchange_rate IS NOT NULL AND rate_from_currency IS NOT NULL AND
         rate_to_currency IS NOT NULL)
        )
);

COMMENT
ON TABLE transaction_fees IS
...
CREATE UNIQUE INDEX idx_portfolios_active_user
    ON portfolios (user_id) WHERE deleted = FALSE;
...
CREATE INDEX idx_transactions_account_symbol
    ON transactions (account_id, execution_symbol) WHERE execution_symbol IS NOT NULL;

-- Recalculation replays only non-excluded transactions
CREATE INDEX idx_transactions_active
    ON transactions (account_id, occurred_at) WHERE excluded = FALSE;

-- Purge job: find expired excluded transactions by date
CREATE INDEX idx_transactions_excluded_purge
    ON transactions (excluded_at) WHERE excluded = TRUE;
