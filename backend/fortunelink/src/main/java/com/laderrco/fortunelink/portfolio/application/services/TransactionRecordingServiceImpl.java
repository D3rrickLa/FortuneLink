...
public class TransactionRecordingServiceImpl implements TransactionRecordingService {
  ...
  @Override
  public Transaction recordFee(Account account, Money amount, FeeType feeType, String notes,
      Instant date) {
    validateIsActive(account);
    validateCashInputs(account, amount, notes, date);
    validateTransactionDate(date);
  ...
  public Transaction recordInterest(Account account, AssetSymbol symbol, Money amount, String notes,
    ...
    Transaction tx = Transaction.builder().transactionId(TransactionId.newId())
        ...
        .cashDelta(amount).fees(List.of()).notes(notes).occurredAt(date).metadata(metadata).build();

    String reason = symbol != null ? REASON_INTEREST + symbol.symbol() : REASON_INTEREST + "CASH";
  ...
  /*
   * The cash effect is amount minus fees. The user transferred in gross, the
   * broker kept their cut. The net is what lands in the account. This is
