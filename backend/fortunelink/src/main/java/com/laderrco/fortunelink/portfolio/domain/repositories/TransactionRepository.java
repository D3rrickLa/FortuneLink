 ...
 * {@link TransactionQueryRepository} in the application layer.
 * This interface owns single-record lookups, mutations, aggregations, and
 * cleanup.
 */
public interface TransactionRepository {
  /**
   * Persists a transaction. {@code portfolioId} is required for new transactions
   * because it is stored as a denormalized column for efficient joins in the
   * recalculation engine. For exclusion/restore updates to existing transactions,
   * the portfolio ID is already on the managed JPA entity — it is still required
   * here to satisfy the contract uniformly and to avoid a secondary lookup.
   *
   * <p>
   * The caller always has the portfolioId available in the command or context
   * object, so passing it here costs nothing and eliminates an extra DB
   * round-trip.
   *
   ...
   */
  Transaction save(Transaction transaction, PortfolioId portfolioId);
   ...
   */
  List<Transaction> findByPortfolioIdAndUserIdAndAccountId(
   ...
   */
  Optional<Transaction> findByIdAndPortfolioIdAndUserIdAndAccountId(
