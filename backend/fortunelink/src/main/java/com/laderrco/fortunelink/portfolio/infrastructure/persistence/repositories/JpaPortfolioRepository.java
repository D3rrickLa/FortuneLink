 ...
 * directly. The rest of the application sees the domain-level
 * {@code PortfolioRepository} interface only.
 */
@Repository
public interface JpaPortfolioRepository extends JpaRepository<PortfolioJpaEntity, UUID> {
   ...
   * managed instances for dirty-checking on save.
   */
   ...
   * Soft-deleted rows ({@code deleted = true}) are intentionally excluded.
   */
  @EntityGraph(attributePaths = {
      "accounts",
      "accounts.positions",
  ...
  /**
   * Ownership check: does this portfolio contain this account?
   * Used by {@code PortfolioLoader.validateAccountOwnershipToPortfolio()}.
   */
  @Query("""
      ...
      WHERE p.id = :portfolioId
        AND a.id = :accountId
      """)
  boolean existsByIdAndAccountId(
  ...
  @Query("""
      ...
      WHERE p.id = :portfolioId
        ...
        AND a.id = :accountId
      """)
  boolean existsByIdAndUserIdAndAccountId(
      @Param("portfolioId") UUID portfolioId,
      @Param("userId") UUID userId,
      @Param("accountId") UUID accountId);

  /**
   * Count of active (non-deleted) portfolios per user.
   * Used to enforce the one-portfolio-per-user limit.
   * <p>
   * IMPORTANT: must exclude soft-deleted rows. See {@code PortfolioRepository}
   * interface Javadoc for the rationale.
   */
  @Query("""
      SELECT COUNT(p)
      FROM PortfolioJpaEntity p
