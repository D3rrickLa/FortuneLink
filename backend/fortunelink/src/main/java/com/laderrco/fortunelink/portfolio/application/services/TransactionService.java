...
public class TransactionService {
  ...
  public TransactionView recordPurchase(RecordPurchaseCommand command) {
    ...
    Transaction recordedTransaction = transactionRecordingService.recordBuy(ctx.account(), symbol,
        assetInfo.type(), command.quantity(), price, command.fees(), command.notes(),
  ...
  public TransactionView recordWithdrawal(RecordWithdrawalCommand command) {
    ...
    Transaction recordedTransaction = transactionRecordingService.recordWithdrawal(ctx.account(),
        command.amount(), command.notes(), command.transactionDate());
  ...
  public TransactionView excludeTransaction(ExcludeTransactionCommand command) {
    ...
    Transaction existing = transactionRepository.findByIdAndPortfolioIdAndUserIdAndAccountId(
    ...
    if (existing.transactionType().affectsHoldings() && existing.execution() != null) {
      eventPublisher.publishEvent(
          new PositionRecalculationRequestedEvent(command.portfolioId(), command.userId(),
              command.accountId(), existing.execution().asset()));
    }
