...
public interface TransactionRecordingService {
        ...
        Transaction recordBuy(Account account, AssetSymbol symbol, AssetType type, Quantity quantity, Price price,
                        List<Fee> fees, String notes, Instant date);

        Transaction recordSell(Account account, AssetSymbol symbol, Quantity quantity, Price price, List<Fee> fees,
         ...
         * Use for exclude/restore recalculation on a single symbol.
         * Caller must call account.clearPosition(symbol) before invoking.
         */
        void replayTransaction(Account account, Transaction tx);

        /**
         * Full replay — applies both position AND cash effects.
         * Use ONLY for full account reconstruction (migration, corruption recovery).
         * 
         * Caller MUST reset both positions and cash to zero before invoking.
         * Calling this without resetting first will double-count everything.
         * 
         * Does NOT return a Transaction - it is applying an existing one, not recording
         * a new one.
         */
