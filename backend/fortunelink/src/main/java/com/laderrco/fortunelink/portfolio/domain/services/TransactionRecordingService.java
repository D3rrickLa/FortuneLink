 ...
 * <p>
 * Each recording method handles the domain logic for a specific
 * transaction type, ensuring that both the {@link Transaction} record is
 * created and the internal
 * account state (positions and cash) is updated accordingly.
 * </p>
 */
public interface TransactionRecordingService {
	 ...
	 * shares).
	 * Cost basis per unit is adjusted automatically; total cost basis is unchanged.
	 *
	 * @param account the account holding the position
	 * @param symbol  the asset being split
	 * @param ratio   the split ratio (e.g., Ratio(2,1) for a 2-for-1 split)
	 * @param notes   audit trail / notes
	 * @param date    the effective date of the split
	 */
	Transaction recordSplit(Account account, AssetSymbol symbol, Ratio ratio, String notes, Instant date);
	 ...
	 * <p>
	 * <b>Note:</b> Cash state is not modified. Use for partial recalculations
