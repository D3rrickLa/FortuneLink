...
public class CommandValidator implements ClassValidation {
    public ValidationResult validate(RecordPurchaseCommand command) {
        ...
        // Instead of converting to LocalDateTime, compare Instants directly
        if (command.transactionDate().isAfter(Instant.now())) {
    ...
    public ValidationResult validate(RecordSaleCommand command) {
        ...
        ValidationResult dateValidation = validateDate(
                LocalDateTime.ofInstant(command.transactionDate(), ZoneOffset.UTC));
    ...
    private boolean isValidSymbol(String symbol) {
        // Basic validation: 1-10 uppercase letters/numbers, possibly with dots or
        // dashes
