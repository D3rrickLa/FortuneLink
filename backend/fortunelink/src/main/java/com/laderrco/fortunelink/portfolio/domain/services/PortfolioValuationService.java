 ...
 *
 * DESIGN CONTRACT: All methods accept a pre-fetched quoteCache. This service MUST NOT call
 * MarketDataService internally. Market data is fetched ONCE at the application service layer and
 * passed in — this prevents double API calls when both a query service and this domain service need
 * the same prices.
 *
 * The caller (application service) is responsible for: 1. Extracting all required symbols 2.
 * Calling MarketDataService.getBatchQuotes() once 3. Passing the resulting map into these methods
 */
public interface PortfolioValuationService {
    /**
     * Calculates the total value of a portfolio converted to the target currency.
     *
     * Sums all account values (each in their own base currency), converting to targetCurrency via
     * ExchangeRateService.
     *
     ...
     * @param quoteCache pre-fetched quotes (must include all symbols in this account)
    ...
    /**
     * Calculates the market value of all open positions in an account. Excludes cash balance.
     *
     * @param account the account entity
     * @param quoteCache pre-fetched quotes (must include all symbols in this account)
