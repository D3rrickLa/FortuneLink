 ...
 * you're targeting (hundreds, not millions), synchronous is fine and far
...
public class CsvImportController {
  ...
  public ResponseEntity<byte[]> getTemplate() {
    byte[] content = csvImportService.generateTemplate()
        .getBytes(StandardCharsets.UTF_8);

    HttpHeaders headers = new HttpHeaders();
    headers.setContentType(MediaType.parseMediaType("text/csv"));
    headers.setContentDisposition(ContentDisposition.attachment()
        .filename("fortunelink_import_template.csv")
        .build());
   ...
   *
   * <p>
   * Returns 200 with a summary on full success.
   * Returns 422 with row-level errors if any row fails validation.
   * Returns 500 if validation passes but commit fails (domain invariant
   * violation).
   *
   * <p>
  ...
  public ResponseEntity<CsvImportResult> importCsv(
    ...
    CsvImportResult result = csvImportService.importTransactions(
        file,
