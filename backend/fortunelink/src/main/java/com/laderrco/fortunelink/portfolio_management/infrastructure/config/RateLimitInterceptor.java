 ...
 * 2. Get or create rate limit bucket for that IP
 * 3. Try to consume 1 token from bucket
 * 4. If successful → allow request
 * 5. If failed → reject with 429 Too Many Requests
 * 
...
public class RateLimitInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        if (probe.isConsumed()) {
            ...
            log.debug("Rate limit check passed for IP: {} (remaining: {})", 
