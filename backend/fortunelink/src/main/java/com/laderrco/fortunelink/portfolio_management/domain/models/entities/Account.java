...
public class Account implements ClassValidation {
    ...
    private Account(AccountId accountId, String name, AccountType accountType, ValidatedCurrency baseCurrency,
        ...
        this.assets = assets != null ? new ArrayList<>(assets) : new ArrayList<>(); // Collections.emptyList returns an
                                                                                    // immutable list
    ...
    public static Account reconstitute(AccountId accountId, String name, AccountType accountType,
            ValidatedCurrency baseCurrency,
            Money cashBalance, List<Asset> assets, List<Transaction> transactions, boolean isActive, Instant closedDate,
            Instant systemCreationDate, Instant lastSystemInteraction) {
    ...
    public static Account createNew(AccountId accountId, String name, AccountType accountType,
            ValidatedCurrency baseCurrency) {
    ...
    public static Account createAccount(AccountId accountId, String name, AccountType accountType,
            ValidatedCurrency baseCurrency, Money initialAmount, List<Asset> assets, List<Transaction> transactions) {
        Instant now = Instant.now();
        return new Account(
                accountId,
                name,
    ...
    void removeTransaction(TransactionId transactionId) {
        ...
        if (removed) {
            ...
            throw new TransactionNotFoundException(
                    String.format("Transaction id, %s cannot be found and deleted.", transactionId.transactionId()));
        }
    }
    ...
    public Transaction getTransaction(TransactionId transactionId) {
        ...
        return this.transactions.stream()
                ...
                .orElseThrow(() -> new TransactionNotFoundException(
                        "Transaction cannot be be found with id, " + transactionId));
    }
    ...
    /*
     * private void applyTransactionToAccount(Transaction transaction, Account
     * account) {
    ...
    private void applyTransaction(Transaction transaction) {
        switch (transaction.getTransactionType()) {
            ...
            case DIVIDEND: // we might need something here for the asset because you know... that is how it
                           // works
            case INTEREST:
            ...
            default:
                ...
                throw new UnsupportedTransactionTypeException(
                        transaction.getTransactionType().toString() + " is not a supported transaction type");
        }
    }

    private void addTransaction(Transaction transaction) { // might need to make this private as we can call
                                                           // recordTransaction, or unless we are saying to just add it
        ClassValidation.validateParameter(transaction);
        boolean alreadyExists = this.transactions.stream()
                .anyMatch(t -> t.getTransactionId().equals(transaction.getTransactionId()));

        if (alreadyExists) {
            throw new IllegalStateException(String.format("Transaction with id %s already exists in this account",
    ...
    private void reduceAssetFromSell(Transaction transaction) {
        Asset asset = this.assets.stream()
                .filter(a -> a.getAssetIdentifier().equals(transaction.getAssetIdentifier()))
                .findFirst()
                .orElseThrow(() -> new AssetNotFoundException("Asset can not be found with given identifier, "
