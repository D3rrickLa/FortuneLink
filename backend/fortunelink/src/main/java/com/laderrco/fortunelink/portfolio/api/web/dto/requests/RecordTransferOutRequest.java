 ...
 *
 * Use this when cash moves OUT of an account to another account.
 * No fees are supported here — if your broker charged an outbound
 * transfer fee, record it as a separate POST /fee with
 * feeType=WIRE_TRANSFER_FEE
 * or ACCOUNT_MAINTENANCE after this transfer-out.
 *
 * See RecordTransferInRequest for the paired inbound side.
 *
 * The account's cash balance must cover the full amount. The server
 * returns 422 INSUFFICIENT_FUNDS if it does not.
 */
