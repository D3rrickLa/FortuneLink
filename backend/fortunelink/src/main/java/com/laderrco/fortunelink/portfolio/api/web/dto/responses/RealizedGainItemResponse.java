 ...
 * <p>
 * Tax note: occurredAt is stored and returned in UTC. The frontend MUST apply the user's timezone
 * offset when bucketing by tax year. A sale at 23:00 UTC on Dec 31 may fall in the next calendar
...
public record RealizedGainItemResponse(
  ...
  public static RealizedGainItemResponse from(RealizedGainRecord record) {
    return new RealizedGainItemResponse(record.symbol().symbol(),
        MoneyResponse.from(record.realizedGainLoss()), MoneyResponse.from(record.costBasisSold()),
        record.occurredAt(), record.isGain(), record.isLoss());
  }
}
