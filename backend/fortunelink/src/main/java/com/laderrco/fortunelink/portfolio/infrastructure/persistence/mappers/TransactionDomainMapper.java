 ...
 * <p>
 * Separate from {@code PortfolioDomainMapper} intentionally. Transactions
 * are not part of the Portfolio aggregate graph — they are loaded independently
 * through {@code TransactionRepository} and have their own lifecycle. Keeping
 * mappers focused on one aggregate boundary prevents them from becoming
 * 800-line god classes.
 * <p>
...
public class TransactionDomainMapper {
  ...
  public Transaction toDomain(TransactionJpaEntity entity) {
    ...
    if (entity.getExecutionSymbol() != null) {
      ...
      execution = new TradeExecution(
          new AssetSymbol(entity.getExecutionSymbol()),
    ...
    if (entity.isExcluded() && entity.getExcludedAt() != null) {
      exclusionRecord = new TransactionMetadata.ExclusionRecord(
          entity.getExcludedAt(),
          UserId.fromString(entity.getExcludedBy().toString()),
          entity.getExcludedReason());
    }

    TransactionMetadata metadata = new TransactionMetadata(
        AssetType.valueOf(entity.getAssetType() != null
            ? entity.getAssetType()
            : AssetType.CASH.name()),
    ...
    // Reconstruct fees
    List<Fee> fees = entity.getFees().stream()
        .map(this::feeToDomain)
        .toList();

    // Reconstruct related transaction ID
    TransactionId relatedId = entity.getRelatedTransactionId() != null
        ? TransactionId.fromString(entity.getRelatedTransactionId().toString())
        : null;

    return Transaction.builder()
        .transactionId(TransactionId.fromString(entity.getId().toString()))
   ...
   * <p>
   * Always creates a fresh entity. Transactions are immutable in the domain
   * (only exclusion state changes), so there is no update path here — use
   * {@link #applyExclusionState} for exclusion mutations on an existing managed
   * entity.
   *
  ...
  public TransactionJpaEntity toEntity(Transaction domain, UUID portfolioId) {
    ...
    UUID relatedId = domain.relatedTransactionId() != null
        ? UUID.fromString(domain.relatedTransactionId().toString())
        : null;

    // Asset type: prefer execution type, fall back to metadata
    String assetTypeName = domain.metadata().assetType() != null
        ? domain.metadata().assetType().name()
        : null;

    TransactionJpaEntity entity = TransactionJpaEntity.create(
        UUID.fromString(domain.transactionId().toString()),
        portfolioId,
        ...
        splitNum,
        splitDenom,
        domain.cashDelta().amount(),
        domain.cashDelta().currency().getCode(),
        domain.metadata().source(),
        domain.isExcluded(),
        excludedAt,
        excludedBy,
        excludedReason,
        domain.metadata().additionalData(),
        domain.notes(),
        domain.occurredAt(),
        relatedId);
    ...
    List<FeeJpaEntity> feeEntities = domain.fees().stream()
        .map(f -> feeToEntity(UUID.randomUUID(), entity, f))
        .toList();
  ...
  /**
   * Applies only the exclusion mutation to a managed JPA entity.
   * Call this instead of {@code toEntity} when restoring or excluding
   * an existing transaction — avoids a pointless re-insert of all fee rows.
   */
  public void applyExclusionState(Transaction domain, TransactionJpaEntity managed) {
    ...
    TransactionMetadata.ExclusionRecord excl = domain.metadata().exclusion();
    if (excl != null) {
      managed.applyExclusionState(
          true,
          excl.occurredAt(),
          UUID.fromString(excl.by().toString()),
          excl.reason());
    } else {
  ...
  private Fee feeToDomain(FeeJpaEntity fe) {
    ...
    if (fe.getAccountAmount() != null && fe.getAccountAmountCurrency() != null) {
      accountAmount = new Money(fe.getAccountAmount(),
          Currency.of(fe.getAccountAmountCurrency()));
    }

    ExchangeRate exchangeRate = null;
    if (fe.getExchangeRate() != null
        && fe.getFromCurrency() != null
        && fe.getToCurrency() != null
        && fe.getExchangeRateDate() != null) {
      exchangeRate = new ExchangeRate(
          Currency.of(fe.getFromCurrency()),
          Currency.of(fe.getToCurrency()),
          fe.getExchangeRate(),
          fe.getExchangeRateDate());
    }

    return new Fee(
        FeeType.valueOf(fe.getFeeType()),
        nativeAmount,
        accountAmount,
        exchangeRate,
        fe.getOccurredAt(),
        new Fee.FeeMetadata(Map.of()) // metadata not persisted — by design
    );
  }

  private FeeJpaEntity feeToEntity(UUID id, TransactionJpaEntity transactionEntity, Fee fee) {
    java.math.BigDecimal accountAmt = fee.accountAmount() != null ? fee.accountAmount().amount() : null;
    String accountAmtCurrency = fee.accountAmount() != null ? fee.accountAmount().currency().getCode() : null;
