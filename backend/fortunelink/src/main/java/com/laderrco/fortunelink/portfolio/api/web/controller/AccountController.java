...
public class AccountController {
  ...
  public AccountView createAccount(@PathVariable String portfolioId,
    ...
    return lifecycleService.createAccount(new CreateAccountCommand(PortfolioId.fromString(portfolioId),
        userId, request.accountName(), request.accountType(), request.strategy(), Currency.of(request.currency())));
  }
   ...
   *
   * Requires the account to have zero positions and zero cash balance.
   * Returns 409 ACCOUNT_CANNOT_BE_CLOSED if these conditions are not met.
   *
   * A closed account can be reopened via PATCH /{accountId}/reopen.
   * Closed accounts remain visible in read queries and their transaction
  ...
  public void closeAccount(@PathVariable String portfolioId, @AuthenticatedUser UserId userId,
      @PathVariable String accountId) {
    lifecycleService.deleteAccount(new DeleteAccountCommand(PortfolioId.fromString(portfolioId),
   ...
   *
   * The account transitions from CLOSED back to ACTIVE. All historical data
   * (transactions, positions, realized gains) is preserved exactly as it was.
   *
   * Use cases:
   * - Account was closed in error
   * - Seasonal account (e.g. annual RRSP contribution account)
   * - Migration: closed old account, then re-importing historical data
   *
   * Returns 409 ACCOUNT_CANNOT_BE_REOPENED if the account is not in CLOSED state
   * (e.g. trying to reopen an ACTIVE or REPLAYING account).
   */
  ...
  /**
   * Returns all accounts in a portfolio with their current positions and
   * balances.
   *
   * This fires one batch market data call for all symbols across all accounts.
   * It is not paginated — the MVP enforces one portfolio per user and the
   * expected account count per portfolio is low (< 20). Add pagination when
   * the multi-portfolio feature ships.
   *
   * Performance note: each call fetches fresh market quotes for all positions.
   * Cache aggressively on the client — the underlying Redis TTL is 5 minutes.
   */
  @GetMapping
  // Spring automatically populates this from query params
  public Page<AccountView> getAllAccounts(@PathVariable String portfolioId, @AuthenticatedUser UserId userId,
      Pageable pageable) {
    return accountQueryService
        .getAllAccounts(new GetAllAccountsQuery(PortfolioId.fromString(portfolioId), userId, pageable.getPageNumber(),
            pageable.getPageSize()));
  }

  /**
   * Returns a single account with its current positions, balances, and market
   * values.
   *
   * Fires a targeted market data batch call scoped to this account's symbols
   * only.
   * Use this for the account detail page. Prefer /portfolios/{id} for a full
   * overview.
   */
