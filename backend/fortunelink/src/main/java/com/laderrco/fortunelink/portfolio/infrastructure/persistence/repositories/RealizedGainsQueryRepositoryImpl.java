...
public class RealizedGainsQueryRepositoryImpl implements RealizedGainsQueryRepository {
    ...
    @Override
    public List<RealizedGainRecord> findByAccountIdAndSymbol(AccountId accountId, AssetSymbol symbol) {
        return map(jpaRepository.findByAccountIdAndSymbol(uuid(accountId), symbol.symbol()));
    }

    @Override
    public List<RealizedGainRecord> findByAccountIdAndYearAndSymbol(
            AccountId accountId, int year, AssetSymbol symbol) {
        return map(jpaRepository.findByAccountIdAndYearAndSymbol(
                uuid(accountId), year, symbol.symbol()));
    }
