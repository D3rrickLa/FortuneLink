...
public class RealizedGainsQueryRepositoryImpl implements RealizedGainsQueryRepository {
  ...
  @Override
  public Page<RealizedGainRecord> findByAccountIdAndYear(AccountId accountId, int year,
      Pageable pageable) {
    return jpaRepository.findByAccountIdAndYear(uuid(accountId), year, pageable)
        .map(this::toDomain);
  }

  @Override
  public Page<RealizedGainRecord> findByAccountIdAndSymbol(AccountId accountId, AssetSymbol symbol,
      Pageable pageable) {
    return jpaRepository.findByAccountIdAndSymbol(uuid(accountId), symbol.symbol(), pageable)
        .map(this::toDomain);
  }

  @Override
  public Page<RealizedGainRecord> findByAccountIdAndYearAndSymbol(AccountId accountId, int year,
      AssetSymbol symbol, Pageable pageable) {
    return jpaRepository.findByAccountIdAndYearAndSymbol(uuid(accountId), year, symbol.symbol(),
        pageable).map(this::toDomain);
  }

  @Override
  public GainsAggregation calculateTotals(AccountId accountId, Integer taxYear,
      AssetSymbol symbol) {
