...
import { useNetWorthHistory } from "@/features/portfolio/queries/usePortfolio";
...
interface PerformanceChartProps {
  currency?: string;
  /**
   * When an account is selected, pass AccountView so the chart can
   * render a cost-basis reference line derived from current positions.
   */
...
export function PerformanceChart({ currency = "USD", account }: PerformanceChartProps) {
  ...
  const { data: snapshots, isLoading, isError } = useNetWorthHistory(getDays(period));
  ...
  // Cost basis reference line from account positions — gives a visual
  // anchor showing what the user paid vs the net worth line.
  ...
  // No snapshots yet — common state for new users.
  // Snapshots are recorded on a daily schedule by the backend.
  // Be explicit rather than showing a confusing empty chart.
      ...
      <CardHeader>
        <div className="flex items-start justify-between">
          <div className="space-y-1">
            ...
            {metrics && (
              <div className={`flex items-center gap-2 text-sm font-semibold ${metrics.isPositive ? "text-green-600" : "text-red-600"
                }`}>
                {metrics.isPositive
                  ? <TrendingUp className="h-4 w-4" />
                  : <TrendingDown className="h-4 w-4" />
                }
                <span>{metrics.isPositive ? "+" : ""}{fmt(metrics.change)}</span>
                <span className="font-normal text-muted-foreground">
                  ({metrics.isPositive ? "+" : ""}{metrics.pct.toFixed(2)}%)
                </span>
              </div>
            )}
            {/* Always clarify scope — this is user-level, not account-level */}
            <p className="text-xs text-muted-foreground">Total net worth · all accounts</p>
          </div>
